...
class Trainer:
    ...
    def run_fn(self, model, train_loader, valid_loader, criterion, optimizer, scheduler,
        ...
        for epoch in range(self.config.engine.epochs):
            ...
            if self._is_main():
                # eval_* expect model(images) -> embeddings
                if eval_groups:
    ...
    def run(self, finetune=False):
        ...
        # ---- Linear LR scaling by number of processes/GPUs ----
        # Only use this if your per-GPU batch size stays constant (global batch grows with world_size).
        # If you keep *global* batch constant (by dividing per-GPU batch by world_size), do NOT scale LR.
        scale = (dist.get_world_size() if self.is_distributed else 1)
        if self._is_main():
            print(f"[Main] LR scale factor = {scale}")

        for attr in ("lr_start", "lr_max", "lr_min"):
            old = float(getattr(config.scheduler_params, attr))
            setattr(config.scheduler_params, attr, old * scale)

        # Optional: keep SWA LR in step with the base LR
        if hasattr(config, "swa_params") and hasattr(config.swa_params, "swa_lr"):
            config.swa_params.swa_lr = float(config.swa_params.swa_lr) * scale
        ...
        num_workers = 0
        timeout = 0  # conservative defaults for cluster environments
        pin_memory = False
        ...
        # Now, the DDP wrapped model is the combined one
        model = model_with_criterion

        optimizer = torch.optim.Adam(
            model.parameters(),  # Optimize ALL parameters of the combined model (backbone + criterion)
        ...
        if config.engine.use_swa:
            swa_model = AveragedModel(unwrap(model_with_criterion))
            swa_model.to(device)
        ...
        # -------------------- Train --------------------
        # IMPORTANT: unwrap to access the same criterion instance used by the combined module
        criterion_for_loop = unwrap(model_with_criterion).criterion
        ...
        if use_wandb_context:
            with WandbContext(config):
                best_score, best_cmc, model = self.run_fn(
                    model, train_loader, valid_loader, criterion_for_loop, optimizer, scheduler,
...
class CombinedModelWithCriterion(nn.Module):
    """
    Wrap backbone + (optionally learnable) criterion so the optimizer/DDP sees both.
    Exposes extract_feat and extract_logits used by eval_fn.
    """
    ...
    # Keep this permissive: let the backbone decide its signature.
    def forward(self, *args, **kwargs):
        return self.model(*args, **kwargs)

    @torch.no_grad()
    def extract_feat(self, *args, **kwargs):
        # If the backbone already provides extract_feat, use it.
        if hasattr(self.model, "extract_feat"):
            return self.model.extract_feat(*args, **kwargs)
        # Fallback: assume forward returns embeddings from images
        return self.model(*args, **kwargs)
